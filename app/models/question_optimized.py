...
# Qdrant 벡터 DB 사용으로 변경 - pgvector 제거
# 벡터 임베딩은 Qdrant 서비스에서 별도 관리
...
class QuestionOptimized(Base):
    ...
    # 벡터 임베딩은 Qdrant에서 별도 관리 (PostgreSQL에서 제거)
    # qdrant_vector_id: Qdrant에서의 벡터 ID 참조용
    qdrant_vector_id = Column(String(100), nullable=True, index=True)  # Qdrant 벡터 ID
...
def migrate_from_old_question(old_question):
    ...
    new_question.modification_info = {
        ...
        # 임베딩 복사 - pgvector에서 qdrant로 마이그레이션
    if hasattr(old_question, 'qdrant_vector_id'):
        new_question.qdrant_vector_id = old_question.qdrant_vector_id
