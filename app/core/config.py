...
class Settings(BaseSettings):
    ...
    # Qdrant 벡터 데이터베이스 설정
    QDRANT_ENABLED: bool = Field(default=True, description="Qdrant 벡터 DB 사용 여부")
    QDRANT_HOST: str = Field(default="localhost", description="Qdrant 서버 호스트")
    QDRANT_PORT: int = Field(default=6333, description="Qdrant 서버 포트")
    QDRANT_API_KEY: Optional[str] = Field(default=None, description="Qdrant API 키 (클라우드용)")
    VECTOR_DIMENSION: int = Field(default=768, description="벡터 차원 수 (DeepSeek 기본)")
