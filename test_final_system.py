...
deepseek_chat_ok = asyncio.run(test_deepseek_chat())

# 6. DeepSeek 임베딩 테스트
print("\n6️⃣ DeepSeek 임베딩 테스트...")
async def test_deepseek_embeddings():
    try:
        test_texts = [
...
deepseek_embed_ok = asyncio.run(test_deepseek_embeddings())
...
qdrant_ops_ok = asyncio.run(test_qdrant_operations())
...
rag_ok = asyncio.run(test_rag_system())
